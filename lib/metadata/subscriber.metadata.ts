import { MetaOrMetaFactory, SbSubscriberMetadataOptions, SbSubscriberTypeMap } from '../interfaces';
...
export class SbSubscriberMetadata<T extends keyof SbSubscriberTypeMap = keyof SbSubscriberTypeMap> {
	...
	constructor(public readonly type: T, metaOptions: SbSubscriberTypeMap[T] extends SbSubscriberMetadataOptions ? MetaOrMetaFactory<SbSubscriberTypeMap[T]> : never) {
		if (typeof metaOptions === 'function') {
			this.metaFactory = metaOptions as any;
			this.metaOptions = {} as any;
		} else {
